...
log = Logger.get_logger(name="playmaker.model.features.long")
    ...
    def to_long(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        for c in OPTIONAL_COLS:
            if c not in m.columns:
                ...
                added.append(c)
        if added:
            log.debug("long.added_optional_cols", extra={"added": added})
        ...
        points = np.where(
            (ftr == "H") & is_home, 3,
            np.where((ftr == "D"), 1, np.where((ftr == "A") & (~is_home), 3, 0))
        )
        long["points"] = points
        ...
        log.debug("long.done", extra={"rows": len(long), "teams": int(long["team"].nunique())})
        return long

...
log = Logger.get_logger(name="playmaker.model.features.rolling")
...
class RollingTeamFeatures:
    ...
    def compute(self, long: pd.DataFrame) -> pd.DataFrame:
        if long.empty:
            log.warning("rolling.empty_input")
            return long

        n_teams = int(long["team"].nunique()) if "team" in long.columns else 0
        log.debug("rolling.start", extra={
            "teams": n_teams,
            "form_window": self.form_window,
            "goal_window": self.goal_window,
            "min_periods_form": self.min_periods_form,
            "min_periods_goal": self.min_periods_goal,
        })
        ...
        out = rolled[["match_key","team","is_home","form5","gf10","ga10","shots5","corn5","yel5","red5","n_prior","rest_days"]]
        log.debug("rolling.done", extra={"rows": len(out)})
        return out

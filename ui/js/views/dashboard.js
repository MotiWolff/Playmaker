...
export async function renderDashboard() {
  ...
  let selected =
    comps.find((c) =>
      (c.name || "").toLowerCase().includes("premier league")
    ) ||
    comps[0] ||
    null;
  ...
  function renderChips() {
    ...
    comps.forEach((c) => {
      ...
      btn.type = "button";
      btn.className = `competition-btn ${
        selected && selected.competition_id === c.competition_id ? "active" : ""
      }`;
      btn.innerHTML = `
        ${
          c.emblem_url
            ? `<img src="${c.emblem_url}" alt="${c.name}" onerror="this.style.display='none'"/>`
            : ""
        }
  ...
  async function loadFixtures() {
    ...
    if (!selected) {
      container.innerHTML =
        '<div class="text-center text-muted">Please select a competition</div>';
      return;
    }
    ...
    try {
      ...
      if (fixtures.length === 0) {
        container.innerHTML =
          '<div class="text-center text-muted">No upcoming fixtures found</div>';
        return;
      }
      ...
      grid.innerHTML = fixtures
        .map((f) => `<div class="col-lg-6 col-xl-4">${fixtureCard(f)}</div>`)
        .join("");
      ...
      container.innerHTML =
        '<div class="text-center text-danger">Error loading fixtures. Please try again.</div>';
    }
  }
